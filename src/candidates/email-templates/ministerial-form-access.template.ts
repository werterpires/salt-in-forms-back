...
/**
 * Template de email para acesso ao formulário do tipo "ministerial"
 *
 * @param ministerialName - Nome do ministerial
 * @param candidateName - Nome do candidato que indicou o ministerial
 * @param accessLink - Link de acesso ao formulário
 * @param accessCode - Código de acesso
 * @returns HTML do email formatado
 */
export function getMinisterialFormAccessEmailTemplate(
  ...
  const contentBeforeButton = [
    ...
    'Por favor, clique no botão abaixo para preencher o formulário de recomendação ministerial.'
  ]
  ...
  return EmailTemplateBuilder.build({
    recipientName: ministerialName,
    contentBeforeButton,
    button: {
      text: 'Preencher Formulário de Recomendação',
      url: accessLink
    },
    contentAfterButton
  })
