...
export class EncryptionService {
  ...
  constructor() {
    if (!process.env.ENCRYPTION_ALGORITHM) {
      throw new InternalServerErrorException('ENCRYPTION_ALGORITHM não está definido no .env')
    }
    if (!process.env.ENCRYPTION_PASSWORD) {
      throw new InternalServerErrorException('ENCRYPTION_PASSWORD não está definido no .env')
    }
    if (!process.env.ENCRYPTION_SALT) {
      throw new InternalServerErrorException('ENCRYPTION_SALT não está definido no .env')
    }
    if (!process.env.ENCRYPTION_KEYLEN) {
      throw new InternalServerErrorException('ENCRYPTION_KEYLEN não está definido no .env')
    }
    if (!process.env.ENCRYPTION_IV_LENGTH) {
      throw new InternalServerErrorException('ENCRYPTION_IV_LENGTH não está definido no .env')
    }
    if (!process.env.ENCRYPTION_BUFFER_FORMAT) {
      throw new InternalServerErrorException('ENCRYPTION_BUFFER_FORMAT não está definido no .env')
    }
    ...
    if (isNaN(keylen)) {
      throw new InternalServerErrorException('ENCRYPTION_KEYLEN deve ser um número válido')
    }

    this.ivLength = parseInt(process.env.ENCRYPTION_IV_LENGTH, 10)
    
    if (isNaN(this.ivLength)) {
      throw new InternalServerErrorException('ENCRYPTION_IV_LENGTH deve ser um número válido')
    }
